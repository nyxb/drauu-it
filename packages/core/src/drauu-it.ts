...
export class DrauuIt {
  ...
  mount(el: string | SVGSVGElement, eventEl?: string | Element) {
    if (this.el)
      throw new Error('[drauu-it] already mounted, unmount previous target first')

    this.el = this.resolveSelector(el)

    if (!this.el)
      throw new Error('[drauu-it] target element not found')
    if (this.el.tagName.toLocaleLowerCase() !== 'svg')
      throw new Error('[drauu-it] can only mount to a SVG element')
    if (!this.el.createSVGPoint)
      throw new Error('[drauu-it] SVG element must be create by document.createElementNS(\'http://www.w3.org/2000/svg\', \'svg\')')
...
export function createDrauuIt(options?: Options) {
  return new DrauuIt(options)
}
